/*
 ...
 Date         : 13.03.2016
 But          : Implementation of the IFileVisitor interface for the quote fetching 
...
public abstract class FileTransformer implements IFileVisitor {
    ...
    public void visit(File file) {
        ...
        try {
            //Create our reader and our writer
            Reader reader = new InputStreamReader(new FileInputStream(file), "UTF-8");
            Writer writer = new OutputStreamWriter(new FileOutputStream(file.getPath() + ".out"), "UTF-8"); // the bug fix by teacher
            //Decorate the writer to apply the desired treatment
            writer = decorateWithFilters(writer);
            //Read and write the data until the end of the file
            int readData;
