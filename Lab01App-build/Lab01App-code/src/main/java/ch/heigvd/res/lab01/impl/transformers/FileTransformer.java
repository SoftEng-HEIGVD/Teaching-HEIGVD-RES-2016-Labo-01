 ...
 * responsibility
 * to open an input text file, to read its content, to apply a number of
 * transformations
...
public abstract class FileTransformer implements IFileVisitor {
    ...
    public void visit(File file) {
        ...
        try {
            ...
            // the bug fix by teacher
            FileOutputStream fos = new FileOutputStream(file.getPath() + ".out");
            Writer writer = new OutputStreamWriter(fos, "UTF-8");
            writer = decorateWithFilters(writer);

            /*
             * There is a missing piece here: you have an input reader and an ouput
             * writer (notice how the
             * writer has been decorated by the concrete subclass!). You need to write a
             * loop to read the
