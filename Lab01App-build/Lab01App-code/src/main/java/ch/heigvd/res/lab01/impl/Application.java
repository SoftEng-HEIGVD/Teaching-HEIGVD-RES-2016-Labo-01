...
public class Application implements IApplication {
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        String pathToFile = WORKSPACE_DIRECTORY;

        // Create directory for the quote
        for (String tag : quote.getTags()) {
            pathToFile += File.separator + tag;
        }
        File f = new File(pathToFile);
        f.mkdirs();

        //Using File output Stream for more performance
        FileOutputStream fs = new FileOutputStream(pathToFile+File.separator + filename);
        fs.write(quote.getQuote().getBytes());
        //fs.flush();
        fs.close();
    }
    ...
    public String getAuthorEmail() {
        return "arthur.verdon@heig-vd.ch";
    }
