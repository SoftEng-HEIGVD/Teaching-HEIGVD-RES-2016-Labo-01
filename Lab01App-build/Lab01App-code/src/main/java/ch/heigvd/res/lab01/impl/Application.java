...
public class Application implements IApplication {
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        List<String> tab = quote.getTags(); //get all quote tags
        String newPath = WORKSPACE_DIRECTORY; //starting point for building directory output.

        for (String dirName : tab) {
            newPath += File.separator + dirName; 
        }
        
        File f = new File(newPath);
        f.mkdirs();
        
        File f2 = new File(newPath + File.separator + filename);
        
        BufferedWriter out = new BufferedWriter( 
        new OutputStreamWriter (new FileOutputStream(f2), charset));
    ...
    public String getAuthorEmail() {
        return "ioannis.noukakis@heig-vd.ch";
    }
