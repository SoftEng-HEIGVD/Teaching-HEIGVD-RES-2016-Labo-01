...
public class Application implements IApplication {
  ...
  public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
    ...
    for (int i = 0; i < numberOfQuotes; i++) {
      ...
      /* storing the content of the quote in a text file (and for generating the directories based on the tags).
       */
      storeQuote(quote, "quote-" + (i+1) + ".utf8");//counting files starts at 1 not at 0
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    //2.get the different quote tag's
    for (String tag : quote.getTags()){
    ...
    //4. f_out : to write into the file
    //stores the quotes into the file
    Writer f_out = new OutputStreamWriter(new FileOutputStream(new File (path + filename)), "UTF-8");
    
    f_out.write(quote.getQuote());
    f_out.close();
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
          try{
              ...
              LOG.log(Level.SEVERE, "An error occured while printing the file's name ",e);
          }
      }
    });
  }
  ...
  public String getAuthorEmail() {
    return new String("yosra.harbaoui@heig-vd.ch");
  }
