...
public class Application implements IApplication {
    ...
    // This constant defines the charset to use for quote files.
    // It is also useful for the file extension.
    ...
    public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
        ...
        for (int i = 0; i < numberOfQuotes; i++) {
            ...
            // The filename convention is the following: 'quote-n.charset' where
            // `n` is the number of the quote and `charset` is the charset used
            // to encode the file.
            String filename = String.format("quote-%d.%s", (i + 1), CHARSET);
            this.storeQuote(quote, filename);
        }
    }
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        ...
        // Then, we create all missing sub-folders.
        new File(path).mkdirs();

        // We append the filename to the path.
        path = path.concat("/" + filename);

        // Then, we write the quote's text into the file.
    ...
    public String getAuthorEmail() {
        return "lucas.elisei@heig-vd.ch";
    }
