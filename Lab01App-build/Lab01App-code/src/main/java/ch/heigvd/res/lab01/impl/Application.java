...
public class Application implements IApplication {
   ...
   void storeQuote(Quote quote, String filename) throws IOException {
      ...
      List<String> tagsList = quote.getTags(); //here i get the tags to create the directory of the file

      //now i create the directory like this : /quotes/A/B/C/quotes-n.utf8
      String separator = File.separator; // to get the default separator of an operating system
      String directory = WORKSPACE_DIRECTORY + separator;
      for (String s : tagsList) {
         directory += s + separator;
      }
      File file = new File(directory); // to create the repertory
      file.mkdirs();// create a directory to the repertory
      Writer fileWriter = new OutputStreamWriter(new FileOutputStream(directory + filename), "UTF-8");// enable to write in in file with the UTF-8 encoding 
      fileWriter.write(quote.getQuote());// write the contains of the quote in the file 
      fileWriter.flush();
   ...
   public String getAuthorEmail() {
      return "norah.sopienoulala@heig-vd.ch";
   }
