...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    for (String t : tags){
      tag.append(File.separator);
      tag.append(t);
    }
    File dir = new File(WORKSPACE_DIRECTORY + tag);
    if(!dir.exists()){
      dir.mkdirs();
    }
    File file = new File(WORKSPACE_DIRECTORY + tag + File.separator +filename);
    if(!file.exists()){
      file.createNewFile();
  ...
  public String getAuthorEmail() {
    return "Thibaut.Loiseau@heig-vd.ch";
  }
