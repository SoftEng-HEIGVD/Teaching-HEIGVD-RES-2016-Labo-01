...
public class Application implements IApplication {
    ...
    public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
        ...
        for (int i = 0; i < numberOfQuotes; i++) {
            ...
            String filname = "quote-" + i + ".utf8"; // edited by me 
            storeQuote(quote, filname);
    ...
    void storeQuote(Quote quote, String filename) throws IOException {
        ...
        String path = WORKSPACE_DIRECTORY + separator;
        ...
        //creates aboresence for tags with the way constriutre above   
        File file = new File(path);

        // we create the directory in which the we will store the file
        file.mkdirs();

        /*to store the quotes we will create a file and used decorators for added functionalities :
         - Fileoupstream to specify which will be used for writing
         - OutputStreamWriter to specify the file encoding , for example utf-8*/
        
        
        Writer storeQuote = new OutputStreamWriter(new FileOutputStream(new File(path + filename)), "UTF-8");

        storeQuote.write(quote.getQuote()); //to store the quotes in the file using the method getQuote
        storeQuote.flush(); //to empty the write buffer to the output medium.
        storeQuote.close(); //close the file
    ...
    public String getAuthorEmail() {
        return "thibaut.toguekam@heig-vd.ch";
    }
