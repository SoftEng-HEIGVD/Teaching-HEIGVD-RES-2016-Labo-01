...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
     ...
     String localPath = Application.WORKSPACE_DIRECTORY;
     /*construction of path*/
     String path = localPath + File.separator;
     List<String> tagList = quote.getTags();
     for(int i=0; i<tagList.size();++i)
        path += tagList.get(i) + File.separator;
     /*creating sub-folder and enter in the new folder*/
     File file = new File(path);
     file.mkdirs();
     /*we store quote into file*/
     path += filename; //final path
     Writer output = new OutputStreamWriter(new FileOutputStream(new File(path)),"UTF-8");
     output.write(quote.getQuote());
     /*we empty the buffer and close */
     output.flush();
     output.close();
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
         ...
         try{
            ...
            LOG.log(Level.SEVERE, "Could not print the name of the file. {0}", ex.getMessage());
         }
  ...
  public String getAuthorEmail() {
     return "annie.dongmongoumnai@heig-vd.ch";
  }
