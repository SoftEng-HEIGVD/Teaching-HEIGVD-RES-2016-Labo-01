...
public class Application implements IApplication {
   ...
   void storeQuote(Quote quote, String filename) throws IOException {
      // Initializes the path with WORKSPACE_DIRECTORY
      String completePath = WORKSPACE_DIRECTORY;

      // Updates the path with the according tags
      for (String s : quote.getTags()) {
         completePath = completePath + "/" + s;
      }

      File folder = new File(completePath);

      // Creates the folders. mkdirs() is used instead of mkdir() to also create parent directories
      folder.mkdirs();

      // Updates the path with the filenam
      String completePathAndFilename = completePath + "/" + filename;

      // Declaration of the writer that will be used to write into the file
      Writer writer;

      try {
         writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(completePathAndFilename), "utf-8"));
   ...
   public String getAuthorEmail() {
      ...
      return "romain.albasini@heig-vd.ch";
   }
