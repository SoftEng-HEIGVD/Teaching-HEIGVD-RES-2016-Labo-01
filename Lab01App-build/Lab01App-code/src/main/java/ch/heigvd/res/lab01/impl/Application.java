...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    // Create folder's path with tags
    for(String tag : quote.getTags()){
      path += File.separator + tag;
    }

    wholePath = new File(WORKSPACE_DIRECTORY + path);

    // Create tags
    wholePath.mkdirs();

    file = new File(wholePath.getCanonicalPath() + File.separator + filename);

    // Once folders are created, create file
    file.createNewFile();

    // Write quote into file
  ...
  public String getAuthorEmail() {
    ...
    return "romain.mercier@heig-vd.ch";
    //throw new UnsupportedOperationException("tThe student has not implemented this method yet.");
  }
