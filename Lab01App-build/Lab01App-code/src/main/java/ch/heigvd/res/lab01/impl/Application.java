 ...
 * Modified by Lara Chauffoureaux on 09.03.2017
 */
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
        ...
        // creation of the directories arborescence
        String directories = WORKSPACE_DIRECTORY;

        for(String tag : quote.getTags()) {

            directories = directories + '/' + tag;
        }
                
        new File(directories).mkdirs();
        
        // file creation      
        String filePath = directories + '/' + filename;
        
        File quoteFile = new File(filePath);
        
        // storage of quote in utf8 file, using the same way that the one explained
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
        ...
        try {
            ...
            // exception have to be caught, thus i reused the code from a previous function
            LOG.log(Level.SEVERE, "Could not write the pass. {0}", ex.getMessage());
            ex.printStackTrace();
        }
      }
    });
  ...
  public String getAuthorEmail() {
      ...
      return "lara.chauffoureaux@heig-vd.ch";
  }
